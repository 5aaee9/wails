...
func (s *windowsSystemTray) run() {
	...
	if s.parent.icon != nil {
		s.lightModeIcon = lo.Must(w32.CreateLargeHIconFromImage(s.parent.icon))
	} else {
		s.lightModeIcon = lo.Must(w32.CreateLargeHIconFromImage(icons.SystrayLight))
	}
	if s.parent.darkModeIcon != nil {
		s.darkModeIcon = lo.Must(w32.CreateLargeHIconFromImage(s.parent.darkModeIcon))
	} else {
		s.darkModeIcon = lo.Must(w32.CreateLargeHIconFromImage(icons.SystrayDark))
	}
	s.uid = nid.UID
...
func (s *windowsSystemTray) setIcon(icon []byte) {
	var err error
	s.lightModeIcon, err = w32.CreateLargeHIconFromImage(icon)
	if err != nil {
		panic(syscall.GetLastError())
	}
	if s.darkModeIcon == 0 {
...
func (s *windowsSystemTray) setDarkModeIcon(icon []byte) {
	var err error
	s.darkModeIcon, err = w32.CreateLargeHIconFromImage(icon)
	if err != nil {
		panic(syscall.GetLastError())
	}
